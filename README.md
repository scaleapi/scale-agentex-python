...
pip install git+ssh://git@github.com/stainless-sdks/agentex-sdk-python.git
```
...
Simply import `AsyncAgentexSDK` instead of `AgentexSDK` and use `await` with each API call:
...
You can enable logging by setting the environment variable `AGENTEX_SDK_LOG` to `info`.

```shell
$ export AGENTEX_SDK_LOG=info
```
...
We are keen for your feedback; please open an [issue](https://www.github.com/stainless-sdks/agentex-sdk-python/issues) with questions, bugs, or suggestions.
