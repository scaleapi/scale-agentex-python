...
from temporalio.contrib.openai_agents import workflow as openai_agents_workflow
...
# MCP server names that should be registered with the worker
MCP_TIME_SERVER = "mcp-server-time"
MCP_WEBSEARCH_SERVER = "openai-websearch-mcp"
MCP_FETCH_SERVER = "mcp-server-fetch"
...
"""}
        ...
        if state_machine_data.task_id and state_machine_data.current_span:
            # Get stateless MCP servers registered with the worker
            time_server = openai_agents_workflow.stateless_mcp_server(MCP_TIME_SERVER)
            websearch_server = openai_agents_workflow.stateless_mcp_server(MCP_WEBSEARCH_SERVER)
            fetch_server = openai_agents_workflow.stateless_mcp_server(MCP_FETCH_SERVER)

            # Create agent with MCP servers
            agent = Agent(
                name="Deep Research Agent",
                instructions=f"""You are a deep research expert that can search the web for information.
...
""",
                mcp_servers=[time_server, websearch_server, fetch_server],
            )

            # Create streaming hooks
            hooks = TemporalStreamingHooks(task_id=state_machine_data.task_id)
            ...
            if hasattr(result, "messages") and result.messages:
                state_machine_data.agent_input_list = result.messages
