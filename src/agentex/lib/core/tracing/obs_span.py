...
def open_obs_span(
    ...
    ``prefer_otel``: open an OTel wrapper first, regardless of ``SGP_OBS_MODE``.
    ...
    """
    try:
        if prefer_otel or get_obs_mode() == LGTM:
            handle = _open_otel_span(name, business_span_id, business_trace_id)
            if handle is not None or not prefer_otel:
                # In lgtm mode a None handle means "no OTel span -> caller uses the
                # ambient fallback". Only when prefer_otel is set (Temporal path)
